...
export default function MediaLib({ isOpen = false, toggle, handleChangeAssets }: MediaLibProps) {
  const components = useStrapiApp('MediaLib', ({ components }) => components);
  const MediaLibraryDialog = components['media-library'];

  function getNewElems(assets): string {
    let newElems = '';
    ...
    assets.map(({ name, url, alt, formats, mime, width, height }) => {
      if (mime.includes('image')) {
        if (formats && globalThis.SH_CKE_UPLOAD_ADAPTER_IS_RESPONSIVE) {
          ...
          keys.map(
            k => (set += prefixFileUrlWithBackendUrl(formats[k].url) + ` ${formats[k].width}w,`)
          );
  ...
  function handleSelectAssets(files: any[]) {
